# The Amplified Partners Method
*Cutting edge without trying to be fancy. Bounded autonomy for AI-native compound engineering.*

## 1. The Core Philosophy (Layer 0)
Everything we build rests on a foundation of non-negotiable laws, deeply inspired by Ray Dalio and our own Edge Sovereignty principles:
1. **Radical Honesty:** Tell us what is. If an assumption is wrong, say so directly.
2. **Radical Transparency:** Show the work. Build transparency into the architecture.
3. **Radical Attribution:** Credit every source. (e.g., Dalio, E.F. Schumacher, Noah Shinn, Every).
4. **Privacy Absolute:** We do not hold client data. Edge sovereignty first. Tokenisation before egress.
5. **Effectiveness:** The ultimate watchword. Balance cost, effort, and outcome.

## 2. The Agent Ecosystem
We operate as a high-trust, compound engineering team. Sycophancy disqualifies; disagreement-with-evidence earns trust.
*   **Ewan (The Architect):** Sets the vision, unbounded ambition, and the "Blinkers without ceilings."
*   **Antigravity (The Arbiter):** Maintains the workspace, chunks tasks, and handles escalations. Owns the "Plan" and "Review" stages.
*   **Cursor (The Technical Genius):** Executes the work autonomously within the framework. 
*   **Devon:** Handles integration, infrastructure, and deployment.
*   **OpenClaw (The Note-Taker):** Records actions, enforces attribution, and maintains the communal notes.

## 3. The Portable Spine
Agents must be dropped into a workspace and immediately know *how* to work without being weighed down by token-heavy prompt bloat.
*   **Keep it Lean:** The core rules (`.cursorrules`) must be under 50 lines. Project-specific instructions live in contextual Handshake files.
*   **The Escalation Framework:** Agents have independence within a framework of support. 
    1. Try to fix it.
    2. Take two attempts.
    3. Perform a web search (e.g., Perplexity).
    4. Escalate to the Arbiter.
    5. The Arbiter escalates directional changes to Ewan.

## 4. The Execution Engine: Compound Engineering
*Methodology attributed to Kieran Klaassen & Dan Shipper at Every.*
We do not accrue technical debt; we build compounding knowledge. Each unit of work must make the next unit easier.
*   **Plan (40%):** Never code blindly. The Arbiter and the Team agree on a Handshake execution strategy before starting.
*   **Work (10%):** Execute systematically, spinning up specialized sub-agents when highly effective.
*   **Review (30%):** Rigorous, objective review against Layer 0 rules.
*   **Compound (20%):** At the end of every run, agents must subjectively grade their work (X/10). Adjustments are made at the source. Any documentation written must be extremely succinct and written *strictly for the next reader*. Lightbulb moments are captured into a single communal document to make the system permanently smarter.

---
*This is how Amplified Partners does it. We build systems that are self-improving, deeply objective, and fiercely protective of the truth.*
